...
# view for dislaing form for choosing two dates, first must be before second or equal. after choosing dates must update table below on the same page with data from query to keitaro
@login_required
def date_range(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            #get user profile buyer_sub_id from user that are logged in
            # get data from query
            query_result = get_result_to_display(query_for_keitaro(start_date, end_date, buyer_sub_id))

            return render(request, template, {'start_date': start_date, 'end_date': end_date, 'query_result': query_result})
    else:
        form = DateRangeForm()
